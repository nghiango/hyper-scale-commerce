...
class ReplicationLagTracker(
  ...
  fun isLagAcceptable(): Boolean = replicaHealthy.get() && currentLagMs.get() <= maxAllowableLagMs
...
class ReplicaLagMonitor(
  ...
  fun sample() {
    try {
      ...
      if (lag == null) lagTracker.setReplicaHealthy(false)
      else lagTracker.recordLag(lag.toLong().coerceAtLeast(0L))
    } catch (_: Exception) {
...
class TransactionRoutingDataSource(
  ...
  public override fun determineCurrentLookupKey(): Any {
    if (TransactionSynchronizationManager.isCurrentTransactionReadOnly()) {
      if (lagTracker.isLagAcceptable()) return DataSourceType.REPLICA
      log.debug(
